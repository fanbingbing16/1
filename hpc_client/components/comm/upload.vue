<template>
	<view class="cl-upload-list" :class="[classList]">
		...
		<!-- 上传列表 -->
		<view class="cl-upload" v-for="(url, index) in urls" :key="index" @tap="choose(index)" :style="{
				height: size[0],
				width: size[1],
			}">
			<image class="cl-upload__target" v-if="url&&(uploadType==='image'||url.indexOf('jpg')>-1||url.indexOf('png')>-1)" :src="url | format" :mode="imageMode" />
			<video class="cl-upload__target"   v-if="url&&(uploadType==='video'&&url.indexOf('jpg')===-1&&url.indexOf('png')===-1)":src="url|format"></video>
			<text class="cl-upload__remove cl-icon-delete-fill" @tap.stop="remove(index)"></text>
		</view>

		<!-- 添加按钮 -->
		<view v-if="isAppend" class="cl-upload" :style="{
				height: size[0],
				width: size[1],
			}" @tap="choose()">
...
<script>
	 ...
	 * @property {Enum} uploadType 上传的类型 可选值image/video
	...
	export default {
		...
		methods: {
			...
			choose(index) {
				...
				console.log(this.uploadType, 'upl')
				if (this.uploadType === 'image') {
					this.chooseImage(count, index)
				} else if (this.uploadType === 'video') {
					this.chooseVideo(count, index)
				}

			},
			chooseVideo(count, index) {
				uni.chooseVideo({
					...
					success: async (res) => {
						...
						if (this.beforeUpload) {
							const valid = await this.beforeUpload(file||res, index);
